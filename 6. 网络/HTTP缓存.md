...
### Last-Modify / If-Modify-Since
	* 浏览器第一次请求某个资源时，服务器返回的header中会加入Last-Modify，表示该资源的最后修改时间。
	* 浏览器再次请求时，请求头中会包含If-Modify-Since，值为之前收到的Last-Modify。服务器收到If-Modify-Since后，判断是否命中缓存。
	* 若命中缓存，则服务器返回HTTP304，并且不会返回Last-Modify。
	* 这种方式不是很准确，所以出现了ETag / If-None-Match。
...
## HTTP访问执行时的缓存流程
第一次访问
![](HTTP%E7%BC%93%E5%AD%98/B3FF9D9D-3148-4943-AFE7-F0D17286C1E5.png)
第二次访问
![](HTTP%E7%BC%93%E5%AD%98/FEFABF57-2C2B-425B-9DE8-C09C7FF7F151.png)